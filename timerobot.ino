...
// When in calibration mode, adjust the following factor until the servos move exactly 90 degrees调整笔写字区间（前后左右）
#define SERVOFAKTOR 650

// Zero-position of left and right servo
// When in calibration mode, adjust the NULL-values so that the servo arms are at all times parallel
// either to the X or Y axis
//调整左舵机和右舵机的初始角度，两个舵机在调试状态下基本是 90 度夹角
...
#define LIFT0 1100 // on drawing surface//（书写、擦除，数字越大越接近白板）
#define LIFT1  750  // between numbers（抬笔）
#define LIFT2 570  // going towards sweeper（返回笔擦时的高度，数字越小抬的越高）
...
Servo servo1;  // lifting servo（定义三个舵机）
...
{ while (Serial.available())
 ...
 temperature= (float)a/1024*105;//温度值
  delay(500); //延时500毫秒
...
number(3, 3, 111, 1);   //擦黑板  
    number(5, 25, a, 0.9);  // 书写a  
    number(19, 25, b, 0.9); //提取并书写b  
